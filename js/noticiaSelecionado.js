//Objeto com todas as informações da noticia
var conteudo = {
  conteudos: [
    {
      titulo: "TITULO1",
      tituloNoticia: "O que é crise Hidrica?",
      linkVideo: "https://www.youtube.com/embed/watch?v=02QNVh8E9r8",
      p1: "As crises hídricas são decorrentes de baixos níveis de água nos reservatórios, quando deveriam estar em níveis normais para atender às necessidades da população.A escassez de água no Brasil se agravou desde 2014, quando a região sudeste foi a mais afetada. A atual crise hídrica do Brasil é considerada a pior da história. Embora o Brasil detenha quase um quinto das reservas mundiais de água, a escassez de água continua sendo uma realidade em muitas partes do país. Alguns estudos sugerem que a escassez de água pode se repetir nos próximos anos.",
      p2: "As causas dessas crises não se limitam à falta irregular de chuvas. São resultado de uma combinação de fatores, incluindo anomalias meteorológicas, mas também má gestão dos recursos hídricos, falta de infraestrutura de abastecimento para atender o crescimento da demanda, educação hídrica, redução do desperdício, uso de fontes alternativas de reservatórios e controle das questões ambientais, principalmente o desmatamento e poluição.",
      p3: "A escassez de água também afeta há muito tempo o semiárido nordestino, onde a seca é um fenômeno natural recorrente. A seca que assola a região desde 2011 é considerada a pior dos últimos 60 anos. Na região, a ação governamental se baseia em operações emergenciais, abastecimento de água e assistência social. Os esforços para promover a atividade econômica adaptada às condições climáticas regionais são insuficientes.",
    },
    {
      titulo: "TITULO2",
      tituloNoticia: "Qual a importância da natureza?",
      linkVideo: "https://www.youtube.com/embed/watch?v=jCmD8GJl4dM",
      p1: "A proteção ambiental é proteger a natureza, independentemente de questões econômicas ou de uso. A ideia da conservação é proteger o meio ambiente das ações humanas. Você já se perguntou quanta água é desperdiçada quando tomamos banhos demorados ou vazamos dias a fio? Atitudes simples podem evitar o desperdício, como reduzir o tempo do banho, aproveitar a água da chuva, reaproveitar a água das máquinas de lavar e lavar carros com baldes de água ao invés de mangueiras. É realmente necessário ter a TV ligada enquanto se usa o computador? Às vezes ligamos vários aparelhos eletrônicos que não usamos, gastando energia desnecessária.",
      p2: "Além disso, as luzes são um grande problema! Quem não manteria uma lâmpada acesa mesmo que não houvesse ninguém na sala? Desligar eletrodomésticos sem uso, desligar as luzes, reduzir o uso do ar-condicionado e substituir as lâmpadas por outras mais econômicas são atitudes que podem reduzir o consumo de energia. É realmente necessário substituir o telefone toda vez que um novo é lançado? Esse computador realmente precisa ser substituído? Às vezes nos deixamos levar pelo consumismo e compramos um monte de coisas inúteis. Com isso, aumentamos a produção de diversos produtos, levando ao uso excessivo e descontrolado dos recursos naturais.",
      p3: "Separar o lixo orgânico do lixo reciclável é fundamental para garantir o descarte adequado de cada produto. Reciclar um produto é muito melhor para o meio ambiente do que fabricar outro porque economizamos recursos naturais e reduzimos a quantidade de lixo no planeta.Jogar lixo nas ruas leva à poluição, doenças e aumento das taxas de inundação. Se não houver latas de lixo por perto, guarde o lixo até chegar em casa.",
    },
    {
      titulo: "TITULO3",
      tituloNoticia: "Porque a amazônia é importante?",
      linkVideo: "https://www.youtube.com/embed/watch?v=_q8uh3lrcns",
      p1: "A floresta amazônica há muito é reconhecida como um tesouro de serviços ecológicos, não apenas para os povos indígenas e comunidades locais, mas também para o resto do mundo. Além disso, de todas as florestas tropicais do mundo, a Amazônia é a única que permanece protegida em tamanho e diversidade. No entanto, o desmatamento na Amazônia está gradualmente desfazendo processos ecológicos frágeis que levaram anos para se estabelecer e aperfeiçoar, à medida que as florestas são queimadas ou derrubadas e o processo de aquecimento global se intensifica.",
      p2: "As árvores desempenham um papel- chave na redução dos níveis de poluição. Para entendermos melhor como isso funciona, vamos tomar como exemplo o gás carbônico( CO2), cujas emissões provêm tanto de fontes naturais como da atividade humana. Nos últimos 150 anos, zilches seres humanos têm lançado quantidades enormes de CO2 no ar por meio da queima de combustíveis fósseis, carvão, petróleo e gás natural – e esta é uma das principais causas das mudanças climáticas no planeta.",
      p3: "O que as florestas tiram do ar, elas devolvem. Quando as florestas queimam, o carbono das árvores é liberado no ar na forma de dióxido de carbono, um gás que polui o ar e já está presente em excesso na atmosfera. O que antes era floresta tropical e savana agora é pasto onde o gado pode ser criado. As pastagens estão infestadas de gado e cupins, cujas atividades metabólicas também liberam dióxido de carbono, embora sua contribuição para a poluição atmosférica ainda seja debatida.",
    },
    {
      titulo: "TITULO4",
      tituloNoticia: "Como é o cenario brasileiro diante das crises hidricas?",
      linkVideo: "https://www.youtube.com/embed/watch?v=kEiUlcxzXr8",
      p1: "Hídricos são as águas superficiais ou subterrâneas disponíveis para qualquer tipo de uso de região ou bacias . Os recursos hídricos podem ser vistos de varias formas, temos que sempre observar se há uma relação com a agropecuária. Pressupõe a valorização da água em função da sua natureza renovável e fluida.De acordo com o Ministério do Meio Ambiente, a bacia Amazônica, sozinha, escoa um quinto do volume de água doce do mundo, o que representa mais de 60% de toda a disponibilidade hídrica do País. O Norte do Brasil conta com outras importantes fontes hídricas superficiais como Tapajós, Negro, Solimões, Madeira e Juruá.",

      p2:"O Brasil possui uma das maiores reservas hídricas do Mundo, concentrando cerca de 12 da água doce superficial disponível no planeta, mamas o contraste na distribuição da água é enorme- A região Norte, com 7 da população, possui 68 da água do País, enquanto o Nordeste, com 29 da população, possui 3, e o Sudeste, com 43 da população, conta com 6. Além disso, problemas como desmatamento das nascentes e a poluição dos rios agravam a situação. Em consequência, 45 da população não têm acesso aos serviços de água tratada e 96 milhões de pessoas vivem sem esgoto sanitário",
      p3:"",
    },
    {
      titulo: "TITULO5",
      tituloNoticia: "Problemas enfrentados com a seca ",
      linkVideo: "https://www.youtube.com/embed/watch?v=KybOtjRDb_o",
      p1: "A seca é um problema que acontece infelizmente até com certa recorrência no Brasil, além de ser uma grande consequência climática, também faz acontecer várias dificuldades sociais para pessoas que habitam regiões que acaba sendo comprometida com esse tipo de situação. Ela também provoca a falta de recursos econômicos, gerando fome e miséria principalmente em regiões como a do sertão. Várias vezes, as pessoas precisam andar durante horas, em meio ao sol e calor forte, para pegar água, muitas vezes ela está suja e contaminada devido as condições. Com uma alimentação precária e consumo de água de péssima qualidade, os habitantes do sertão nordestino acabam sendo vítimas de muitas doenças.",
      p2: "Uma das piores secas que já ocorreu no Brasil inclusive foi a seca que acometeu o sertão nordestino no início de 2012, trouxe diversos prejuízos para a economia local, incluindo a grande perda na pecuária e em produtos agrário como o milho e o feijão, já na pecuária fez com que muitos gados viessem a óbito graças a falta de água.",
      p3: "",
    },
    {
      titulo: "TITULO6",
      tituloNoticia: "Brasil e o descaso com a natureza",
      linkVideo: "https://www.youtube.com/embed/watch?v=wla6ppfBBaM",
      p1: "O meio ambiente tem diversas importâncias diferentes para a população e que impactam de maneira muito grandiosa a vida dos seres humanos, afinal é de onde tiram os recursos essenciais para a manutenção da vida na terra, e os problemas ambientais já começam a causar um desequilíbrio nesse sistema.",
      p2: "O Brasil sofreu muito e sofre com problemas causados por sua própria população, seja ela em todos os ambitos, desde ao menino que joga um canudo plástico ou um saco de salgadinho em rios e mares, até aos madeireiros que provocam desmatamento e fazem queimadas na Amazônia e em regiões florestais no território brasileiro. O pior de tudo em cima disso é a lentidão que esses casos são tratados no Brasil pelo governo, uma crise ambiental instaurada ao longo de vários anos em meio a esse descaso. Um número impactante em meio a isso, é que os números do desmatamentos e queimadas só fazem crescer ao longo do tempo no Brasil.",
      p3: "",
    },
    {
      titulo: "TITULO7",
      tituloNoticia: "Maiores fontes hídricas do Brasil",
      linkVideo: "https://youtube.com/embed/dBVzhtTpH-o",
      p1: "Sem dúvidas alguma é claro que a maior fonte hídrica do Brasil se encontra no norte, na região amazônica, cerca de 80% da água superficial se encontram na região que por outro lado não apresenta tanta demanda de uso por apresentar uma baixa densidade demográfica. Enquanto isso temos diversos municípios no nordeste e em algumas regiões específicas como em Minas Gerais que sofrem bastante com a falta desse recursos, enfrentando vários períodos longos de estiagem.",
      p2: "O norte também tem outras fontes hídricas bastante importantes, como o rio Tapajós, Rio Negro, Solimões e Madeira. Sem contar também o aquifero de alter do chão que existe na região, que é um dos maiores reservatórios de água subterrâneo não só do Brasil como do mundo. Já existem até algumas pesquisas realizadas que chegaram a conclusão que todos os estados do norte são considerados abundantes no quesito de recursos hídricos, gerando pra cada um de seus habitantes um enorme volume de água.",
      p3: "",
    },
  ],
};

//Filtra a noticia escolhida pelo url
const urlParams = new URLSearchParams(window.location.search);
const id_usuario = urlParams.get("id");
conteudo = conteudo.conteudos.filter((obj) => obj.titulo == id_usuario)[0];

//Pega as tags e troca dependendo da tag colocada

var titulo = (document.querySelector(".titulo-noticia").textContent =
  conteudo.tituloNoticia);
var p1 = (document.querySelector(".p1").textContent = conteudo.p1);
var p2 = (document.querySelector(".p2").textContent = conteudo.p2);
var p3 = (document.querySelector(".p3").textContent = conteudo.p3);
var iframe = (document.querySelector(".iframe").src = conteudo.linkVideo);
